...
function FlashcardEditor({ flashcards, setFlashcards, addFlashcard }) {
  ...
  const inactivateFlashcard = (id, index) => {
    axios.put(`http://localhost:5001/api/flashcards/${id}/inactivate`)
      .then(response => {
        const updatedFlashcards = flashcards.filter((_, i) => i !== index);
        setFlashcards(updatedFlashcards); // Remove from the frontend view
      })
      .catch(err => {
        console.error('Error inactivating flashcard:', err);
        setError('Failed to inactivate flashcard.');
      });
  };
        ...
        <tbody>
          {flashcards.map((card, index) => (
            <tr key={index} className="border-b">
              ...
              <td className="px-4 py-2">
                <button
                  onClick={() => inactivateFlashcard(card.id, index)} // Or deleteFlashcard if you want to delete
